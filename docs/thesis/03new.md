...
编码器使用长度为 $L=3$ 的滑动窗口，将连续 3 个时间点的全局观测组成输入 $t\in\mathbb{R}^{3\times 48}$。编码器训练所使用的异常标签由仿真器故障记录经 **ADE** 机制生成（见 2.3.5 节），以保证与数据收集阶段一致。
...
在时间维度上，本文以窗口 $t\in\mathbb{R}^{L\times 48}$ 的形式输入近期 $L=3$ 个调度间隔的全局观测，并通过 GRU 提取其短期动态特征。为便于说明，以下给出 GRU 的标准递推形式。设第 $t$ 个时间步的输入为 $x_t$，隐藏状态为 $h_{t}$，则有：
...
$$
\text{score}=0.8\cdot E + 0.2\cdot R
$$
...
// 阶段2（编码器离线训练）
1:  按训练集每维最大值对时间序列归一化
2:  为每步构建窗口 t ∈ R^{3×48}，加载调度矩阵 s ∈ R^{16×16}
...
11:     从环境获取当前窗口 t 与当前调度 s
12:     异常分数, 原型向量 = 编码器(t, s)
13:     if 无异常 then continue
14:     嵌入 = 按异常标记掩码原型（正常节点置 0）
15:     s_new = 生成器(嵌入, s)
16:     score_new = 0.8×能耗(s_new) + 0.2×延迟(s_new)
17:     score_org = 0.8×能耗(s) + 0.2×延迟(s)
18:     y = [0,1] 若 score_new ≤ score_org 否则 [1,0]
